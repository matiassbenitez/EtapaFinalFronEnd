    ...
    return (
        <div className="container mx-auto p-4 sm:p-6 lg:p-8 max-w-4xl bg-white shadow-xl rounded-lg my-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <h2 className="text-xl font-semibold text-indigo-600 border-l-4 border-indigo-600 pl-3 py-1 bg-indigo-50/50">Datos Personales</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                    
                    {/* Nombre */}
                    <FormInput label="Nombre" name="nombre" value={formData.nombre} onChange={handleChange} type="text" required />
                    ...
                    {/* Número de Documento */}
                    <FormInput label="Número de Documento" name="nroDoc" value={formData.nroDoc} onChange={handleChange} type="text" required />

                    {/* Fecha de Nacimiento */}
                    <FormInput label="Fecha de Nacimiento" name="fechaNacimiento" value={formData.fechaNacimiento} onChange={handleChange} type="date" required />

                    {/* Nacionalidad */}
                    <FormInput label="Nacionalidad" name="nacionalidad" value={formData.nacionalidad} onChange={handleChange} type="text" required />

                    {/* Ocupación */}
                    <FormInput label="Ocupación" name="ocupacion" value={formData.ocupacion} onChange={handleChange} type="text" required />

                    {/* Posición IVA */}
                    <FormInput label="Posición IVA" name="posIva" value={formData.posIva} onChange={handleChange} type="text" required />
                </div>

                {/* --- Separador y Sección de Contacto --- */}
                <hr className="my-8 border-gray-200" />
                <h2 className="text-xl font-semibold text-indigo-600 border-l-4 border-indigo-600 pl-3 py-1 bg-indigo-50/50">Datos de Contacto y Domicilio</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                    ...
                    {/* Teléfono */}
                    <FormInput label="Teléfono" name="telefono" value={formData.telefono} onChange={handleChange} type="text" required />

                    {/* Domicilio (Ocupa toda la fila en md+) */}
                    <div className="md:col-span-2">
                        <FormInput label="Domicilio" name="domicilio" value={formData.domicilio} onChange={handleChange} type="text" required />
                    </div>

                    {/* País */}
                    <FormInput label="País" name="pais" value={formData.pais} onChange={handleChange} type="text" required />
...
const FormInput: React.FC<FormInputProps> = ({ label, name, value, onChange, type = 'text', required = false, ...props }) => (
    <div>
        <label
