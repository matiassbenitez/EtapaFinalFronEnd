...
export default function FacturacionPage() {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        const dataReserva = await getReservaById(Number(reservaId));
                    ...
                    <tbody>
                        {itemsPendientes.map((s, idx) => (
                            <tr key={`pend-${s.id}-${idx}`} className="border-b">
                                <td className="p-2 text-center"><input type="checkbox" checked={!!selectedServicios[s.id]} onChange={() => setSelectedServicios(prev => ({...prev, [s.id]: !prev[s.id]}))} /></td>
